...
import { RefreshCw, ShieldAlert, Clock3, Activity } from "lucide-react";

export const IncidentCenterPage: React.FC = () => {
  const { securityEvents, refreshSecurityEvents } = useApi();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        {securityEvents.length === 0 ? <div className="p-12 text-center text-slate-500"><ShieldAlert className="w-10 h-10 mx-auto mb-3 text-slate-700" /><p className="text-sm">No security events yet.</p><p className="text-xs mt-1">Run a scenario from Message Security to create the first event.</p></div> : <div className="divide-y divide-slate-800">{securityEvents.map((event) => <div key={event.event_id} className="p-5 hover:bg-slate-950/40 transition"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div className="flex items-start gap-3"><div className="p-2 rounded-lg bg-cyan-950/50 border border-cyan-900"><ShieldAlert className="w-4 h-4 text-cyan-400" /></div><div><div className="flex flex-wrap items-center gap-2"><span className="font-bold text-sm">{event.category}</span><span className="text-[10px] px-2 py-1 rounded-full bg-slate-800 text-slate-300">{event.severity}</span><span className="text-[10px] px-2 py-1 rounded-full bg-slate-800 text-cyan-300">{event.action}</span></div><p className="text-xs text-slate-400 mt-2">{event.reason}</p><div className="flex items-center gap-4 mt-2 text-[10px] text-slate-600 font-mono"><span>ID {event.event_id.slice(0, 12)}</span><span><Clock3 className="inline w-3 h-3 mr-1" />{new Date(event.timestamp).toLocaleString()}</span></div></div></div><div className="grid grid-cols-3 gap-2 min-w-[260px]"><Score label="MSG" value={event.scores.message} /><Score label="NET" value={event.scores.network} /><Score label="TOTAL" value={event.scores.composite} /></div></div>{event.evidence?.length ? <div className="mt-4 flex flex-wrap gap-2">{event.evidence.map((x, i) => <span key={i} className="text-[10px] px-2 py-1 rounded-md bg-slate-950 border border-slate-800 text-slate-400">{x}</span>)}</div> : null}</div>)}</div>}
      </div>
    </div>
  );
};
